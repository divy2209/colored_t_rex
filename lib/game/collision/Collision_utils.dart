...
import 'package:colored_t_rex/game/t_rex/t_rex.dart';
...
bool checkForCollision(Obstacle obstacle, TRex tRex){
  ...
  if(obstacleBox.toRect().overlaps(tRexBox.toRect())){
    ...
    for(final obstacleCollisionBox in collisionBoxes) {
      final adjObstacleBox = createAdjustedCollisionBox(obstacleCollisionBox, obstacleBox);

      for(final tRexCollisionBox in tRexCollisionBoxes){
        final adjTRexBox = createAdjustedCollisionBox(tRexCollisionBox, tRexBox);
        crashed = crashed || adjTRexBox.toRect().overlaps(adjObstacleBox.toRect());
      }
    }
    return crashed;
  }
