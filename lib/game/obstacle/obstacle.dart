...
class ObstacleManager extends PositionComponent with HasGameRef<TRexGame> {
  ...
  void update(double dt){
    ...
    if(children.isNotEmpty){
      ...
      if(lastObstacle != null &&
          ...
          (lastObstacle.x+lastObstacle.width+lastObstacle.gap)<dimensions.width){
  ...
  void addNewObstacle(double speed){
    ...
    if(duplicateObstacleCheck(type) || speed<type.multipleSpeed) {
      ...
      if(history.length>1){
        final sublist = history.toList().sublist(0, gameRef.config.maxObstacleDuplication);
...
class Obstacle extends SpriteComponent with HasGameRef<TRexGame> {
  Obstacle({
    ...
    if(internalSize>1 && type.multipleSpeed>speed) internalSize = 1;
    width = type.width*internalSize;
    height = type.height;
    gap = computeGap(gapCoefficient, speed);
    final actualSrc = sprite!.src;
    sprite!.src = Rect.fromLTWH(actualSrc.left, actualSrc.top, width, actualSrc.height);
  }
  ...
  late int internalSize = getRandomNum(1.0, config.maxObstacleLength/1).floor();
  ObstacleType type;
