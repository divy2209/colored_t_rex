...
import 'package:colored_t_rex/game/t_rex/config.dart';
...
class TRex extends PositionComponent with HasGameRef<TRexGame>{
  ...
  late final SurprisedTRex surprisedTRex = SurprisedTRex(gameRef.spriteImage, config);
...
class TRexStateStillComponents extends SpriteComponent with TRexStateVisibility {
  TRexStateStillComponents({
...
class TRexStateAnimationComponent extends SpriteAnimationComponent with TRexStateVisibility {
  TRexStateAnimationComponent({
    required List<TRexStatus> showFor,
    required Image spriteImage,
    required TRexConfig config,
...
class RunningTRex extends TRexStateAnimationComponent {
  RunningTRex(
      Image spriteImage,
      TRexConfig config
  ) : super(
...
class WaitingTRex extends TRexStateStillComponents {
...
class JumpingTRex extends TRexStateStillComponents {
...
class SurprisedTRex extends TRexStateStillComponents {
  SurprisedTRex(Image spriteImage, TRexConfig config)
      : super(
          showFor: [TRexStatus.crashed],
          config: config,
          srcPosition: Vector2(1782, 6.0),
          spriteImage: spriteImage
  );
}