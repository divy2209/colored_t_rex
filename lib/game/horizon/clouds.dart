...
class Cloud extends SpriteComponent {
  ...
  }) : cloudGap = getRandomNum(config.minCloudGap, config.maxCloudGap),
       super(
        sprite: Sprite(
          spriteImage,
  ...
  void onGameResize(Vector2 gameSize){
    ...
    y = ((absolutePosition.y/2-(config.maxSkyLevel-config.minSkyLevel)) +
              getRandomNum(config.minSkyLevel, config.maxSkyLevel)) - absoluteParentPosition.y;
  }
}

class CloudManager extends PositionComponent with HasGameRef<TRexGame> {
  CloudManager({required this.horizonConfig});
  ...
  void addCloud() {
    ...
    cloud.y = ((absolutePosition.y/2-(cloudConfig.maxSkyLevel-cloudConfig.minSkyLevel)) +
                  getRandomNum(cloudConfig.minSkyLevel, cloudConfig.maxSkyLevel)) - absolutePosition.y;
    addChild(cloud);
  }

  double get cloudSpeed => horizonConfig.bgCloudSpeed/1000*gameRef.currentSpeed;
